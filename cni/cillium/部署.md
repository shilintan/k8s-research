...
```shell
helm install cilium cilium/ --namespace=kube-system -f values.yaml
helm upgrade cilium cilium/ --namespace=kube-system -f values.yaml
...
kubectl -n kube-system describe pod cilium-ggpw8
...
kubectl -n kube-system logs -f --tail 300 cilium-nxp6l config
